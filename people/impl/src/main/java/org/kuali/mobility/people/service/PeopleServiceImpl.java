...
public class PeopleServiceImpl implements PeopleService {
	...
	public List<Person> performSearch(Search search) {
		...
		try {
			...
			// // search.setErrorMessage("Too many results, showing first " +
			// persons.size() + ":");
	...
	private boolean ferpaRestricted(AdsPerson adsPerson) {
		...
		if (ferpa > 0 && ((ferpa & 31) != 0 || (ferpa & 121) != 0)) {
			return true;
			// LOG.info("Skipping entry for " +
			// this.getStringAttribute(adsPerson, "cn") +
			// " due to FERPA restriction " + ferpa);
		}
		return false;
	}
	...
	private boolean convertAdsPerson(AdsPerson adsPerson, Person person) {
		...
		if (adsPerson.getIuEduPersonAffiliation() != null) {
			// for (String affiliation : (List<String>)
			// adsPerson.getIuEduPersonAffiliation()) {
		...
		if (person.getAffiliations().size() < 1) {
			// LOG.info("Skipping entry for " +
			// this.getStringAttribute(adsPerson, "cn") +
			// " due to no remaining affiliations. " +
			// this.getStringAttribute(adsPerson, "displayName"));
			return false;
		}
		return true;
	}
