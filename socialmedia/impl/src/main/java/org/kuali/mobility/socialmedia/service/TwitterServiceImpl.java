...
public class TwitterServiceImpl implements TwitterService {
	...
	private static Thread feedReloaderThread = null;
	...
	public void startCache() {
		feedReloaderThread = new Thread(new FeedReloader());
		feedReloaderThread.setDaemon(true);
	...
	public List<Tweet> retrieveCombinedFeeds(List<String> publicIds) {
		...
		int feedCount = 25;
		try{
			feedCount = Integer.parseInt(configParamService.findValueByName("SocialMedia.Twitter.TweetCount"));
		} catch (Exception e) {
	...
	private TwitterFeed retrieveFeed(String publicId) {
		return twitterFeeds.get(publicId);
	}
	...
	private class FeedReloader implements Runnable {
            ...
            while (true) {
                ...
                if (now.after(nextCacheUpdate)) {
                    try {
                    	...
                    	LOG.error("Error reloading feed cache.", e);
                    }
                    updateCalendar.add(Calendar.MINUTE, FEED_UPDATE_INTERVAL);
        ...
        private void reloadFeeds() {
        	...
        	String[] feedList = feedString.split(",");
        	for (String feed : feedList) {
        		updateFeed(feed);
        	}
        }
        
        private void updateFeed(String feed) {
        	TwitterFeed twitterFeed = twitterFeeds.get(feed);
    		if (twitterFeed == null) {
    			twitterFeed = new TwitterFeed();
    			twitterFeed.setTwitterId(feed);
    			TwitterFeed existing = twitterFeeds.putIfAbsent(feed, twitterFeed);
    			if (existing == null) {
    				updateFeed(twitterFeed);
    			} else {
    				updateFeed(existing);
    			}
    		} else {
