...
<spring:message code="orientation.title" var="title"/>
<kme:page title="${title}" id="orientation" backButton="true" homeButton="true">
	<kme:content>
		<kme:listView id="menulist" dataTheme="c" dataDividerTheme="b" filter="false">
			<script type="text/javascript">
				$('[data-role=page][id=orientation]').live('pagebeforeshow', function(event, ui) {
					$('#menuListTemplate').template('menuListTemplate');
					refreshTemplate('${pageContext.request.contextPath}/orientation?lang=${localeCode}', '#menulist', 'menuListTemplate', '<li>No Menus</li>', function() {$('#menulist').listview('refresh');});
				});
			</script>
